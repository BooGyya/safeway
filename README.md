...
실제 구현 기준 ERD입니다. 회원 · 경로(경로/즐겨찾기/히스토리) · 커뮤니티(게시글/댓글/좋아요/사진/팔로우/공지) · 챗봇 대화 테이블로 구성됩니다. 인프라 시설 데이터(신호등 44,020건 · V2X 교차로 2,779건 · 엘리베이터 · 이동지원센터 등 5개 테이블)는 FK 없이 위경도 공간 검색으로 참조되어 다이어그램에서 생략했습니다.
