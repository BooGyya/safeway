...
INTERSECT_SHP = r'C:\Users\SSAFY\Desktop\safesafe\DATA\A008_P\A008_P.shp'


# 교차로 데이터 로딩
print("교차로 데이터 로딩...")
intersections = []
sf_int = shapefile.Reader(INTERSECT_SHP, encoding='cp949')
for r in sf_int.iterRecords():
    try:
        name = r['CSS_NAM']
        x, y = r['XCE'], r['YCE']
        if x and y and name and str(name).strip():
            lng, lat = transformer.transform(float(x), float(y))
            if 37.4 <= lat <= 37.7 and 126.7 <= lng <= 127.2:
                intersections.append((lat, lng, str(name).strip()))
    except:
        pass
print(f"교차로: {len(intersections)}개 로딩 완료")


def get_nearest_intersection(lat, lng, max_dist=0.005):
    """가장 가까운 교차로명 반환 (약 500m 이내)"""
    min_dist = float('inf')
    nearest = None
    for ilat, ilng, name in intersections:
        dist = math.sqrt((lat - ilat)**2 + (lng - ilng)**2)
        if dist < min_dist:
            min_dist = dist
            nearest = name
...
with open(SIGNAL_CSV, encoding='euc-kr') as f:
    ...
    for i, row in enumerate(reader):
        try:
            ...
            lng, lat = transformer.transform(x, y)
            if 37.4 <= lat <= 37.7 and 126.7 <= lng <= 127.2:
                road_nm = get_nearest_intersection(lat, lng)
